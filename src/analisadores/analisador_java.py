...
class AnalisadorJava(AnalisadorBase):
    ...
    def _verificar_recursao(self, ast_tree):
        nomes_metodos = [node.name for path, node in ast_tree.filter(tree.MethodDeclaration)]
        for path, node in ast_tree.filter(tree.MethodInvocation):
            if node.member in nomes_metodos:
                return ["Uso de Recursividade"]
        return []

    def _verificar_funcoes_prontas(self, ast_tree, lista_proibida):
        violacoes = []
        for path, node in ast_tree.filter(tree.MethodInvocation):
    ...
    def _verificar_condicionais(self, ast_tree):
        violacoes = []
    ...
    def _verificar_importacoes(self, ast_tree):
        violacoes = []
